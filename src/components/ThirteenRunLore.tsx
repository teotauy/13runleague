      ...
      <div className="rounded-lg border border-gray-800 bg-[#111] p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-gray-700 font-mono">
            {totalYears} seasons · peak {peakYearEntry[0]} ({peakYearEntry[1].toLocaleString()} games)
          </span>
        </div>

        {/* Sparkline-style bar chart — one bar per year */}
        <div className="flex items-end gap-px h-16 w-full">
          {yearOrdered.map(([yr, count]) => {
            const heightPct = maxYear > 0 ? (count / maxYear) * 100 : 0
            const isLeagueEra = yr >= 2018
            return (
              <div
                key={yr}
              ...
              >
                <div
                  className={`w-full rounded-t-sm transition-all ${
                    isLeagueEra ? 'bg-[#39ff14]' : 'bg-gray-700'
                  }`}
                  style={{ height: `${heightPct}%`, minHeight: count > 0 ? '2px' : '0' }}
                />
                {/* Tooltip on hover */}
                <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 bg-[#0a0a0a] border border-gray-700 rounded px-1.5 py-0.5 text-xs font-mono text-white whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-10">
                  {yr}: {count.toLocaleString()}
                </div>
              </div>
            )
          })}
        ...
        {/* Year axis labels — just first, midpoint, last */}
        <div className="flex justify-between text-xs text-gray-700 font-mono mt-1.5">
          <span>{firstYear}</span>
          <span className="text-gray-600 text-xs">
            green = South Brooklyn era (2018–)
          </span>
          <span>{lastYear}</span>
        </div>
      </div>
